...
class UserProfile extends Component {
  ...
  selectOffice = (e) => {
    ...
    this.setState({
      ...
      selectedOfficeHours: value !== "" ? this.state.user.offices[value].officeHours : ""
    })
  }

  onOfficeFormDisplay = () => {
    let newOffice = {
      id: this.state.user.offices ? (this.state.user.offices[this.state.user.offices.length-1].id + 1) : 0,
      campusName: "",
      buildingName: "",
      officeNum: "",
      address: "",
  ...
  addOfficeHours = () => {
      let form = {
          id: this.state.selectedOfficeHours ? (this.state.selectedOfficeHours[this.state.selectedOfficeHours.length-1].id + 1) : (this.state.addedOfficeHours[this.state.addedOfficeHours.length-1].id + 1),
          days: {
            monday: "",
            tuesday: "",
            wednesday: "",
      ...
      if(this.state.selectedOfficeHours){
          ...
          let addedOfficeHours = [...this.state.user.offices[officeSelected].officeHours];
        ...
        <div id="message">
          <div id="form" className='form'>
            <MDBFormInline id="forms" onSubmit={this.handleSubmit}>
              <MDBInput style={{color: 'white'}} type='text' onChange={this.handleChange} value={this.state.user.username} name='username' label='Name' required={true}/>
              ...
              <select style={{backgroundColor: 'black', color: 'white'}} name="degree" value ={this.state.user.degree} onChange={this.handleChange} required={true}>
                <option value="">Degree</option>
                <option value="B.A.">B.A.</option>
                <option value="B.S.">B.S.</option>
              </select>
              <MDBInput style={{color: 'white'}} type='text' onChange={this.handleChange} value={this.state.user.university} name='university' label='University' required={true}/>
              <MDBInput style={{color: 'white'}} type='text' onChange={this.handleChange} value={this.state.user.college} name='college' label='College' required={true}/>
              <MDBInput style={{color: 'white'}} type='text' onChange={this.handleChange} value={this.state.user.department} name='department' label='Department' required={true}/>
                <div>
                  <h1 style={{color: 'white'}}>Office Details</h1>
                  {!this.state.addNewOffice ? (
                    <select style={{backgroundColor: 'black', color: 'white'}} name="office" value={this.state.officeSelected} onChange={this.selectOffice}>
                        <option value="">Select office to edit</option>
                        {this.state.user.offices ?
                          this.state.user.offices.map((office, i) => {
                            return (
                              <option key={i} value={i}>{office.campusName} - {office.buildingName}</option>
                            )
                          })
                  ...
                  {this.state.officeSelected != "none" ?
                    officeForm : ""
                  }
                </div>
              <br />
              <div hidden={newOfficeButton}>
                <Button style={{marginTop: "2px"}} type="button" bsStyle="danger" onClick={this.onOfficeFormDisplay}>Add new office</Button>
              </div>
              ...
              <Button bsStyle="danger" type='button' onClick={this.cancel}>Cancel</Button>
            </MDBFormInline>
          </div>
        </div> :
        this.props.user ? (
          <div id="message">
            <div>
                              ...
                              <div className="officeHours" key={i}>
                                ...
                                {Object.keys(hour.days).sort((a,b) => {
                                  ...
                                  if(hour.days[day] === true) {
                                    return (
                                      <li key={i}>{day.charAt(0).toUpperCase()+ day.slice(1)}</li>
                                    )
                                  }
                                })}
                                </ul>
                                <h3>Time:</h3>
                                <h3>{hour.start}PM to {hour.end}PM</h3>
                              </div>
                            </div>
                          )
                        })}
              ...
              <hr />
              <div>
                {this.props.user.phone ? <h3>Phone: {this.props.user.phone}</h3> : ""}
                {this.props.user.volP ? <h3>VolP: {this.props.user.volP}</h3> : ""}
                {this.props.user.facebook ? <h3>Facebook: {this.props.user.facebook}</h3> : ""}
                {this.props.user.twitter ? <h3>Twitter: {this.props.user.twitter}</h3> : ""}
                {this.props.user.linkedIn ? <h3>LinkedIn: {this.props.user.linkedIn}</h3> : ""}
              </div>
              <hr />
